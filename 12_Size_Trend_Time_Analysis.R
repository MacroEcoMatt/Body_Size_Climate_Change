...
resid_weights <- data.frame(Binomial = mass_bird$Binomial, resid = b_m_bisqr$resid, weight = b_m_bisqr$w)
...
resid_l <- data.frame(Binomial = length_bird$Binomial, resid = b_l_bisqr$resid, weight = b_l_bisqr$w)
resid_l <- resid_l[order(b_l_bisqr$w),]
fitted_vall <- fitted(b_l_bisqr)
Binomial <- length_bird$Binomial
...
resid_s <- data.frame(Binomial = size_bird$Binomial, resid = b_s_bisqr$resid, weight = b_s_bisqr$w)
resid_s <- resid_s[order(b_s_bisqr$w),]
fitted_vals <- fitted(b_s_bisqr)
Binomial <- size_bird$Binomial
...
bma <- ggpredict(b_m_bisqr, terms=c("AI_trend  [-1:1 by=0.01]"))
...
bla <- ggpredict(b_l_bisqr, terms=c("AI_trend [-0.1:0.1 by=0.01]"))
...
#size
bst <- ggpredict(b_s_bisqr, terms=c("TPI_trend [-0.01:0.01 by=0.001]"))
...
bsa <- ggpredict(b_s_bisqr, terms=c("AI_trend [-0.2:0.2 by=0.01]"))
...
bsh <- ggpredict(b_s_bisqr, terms=c("HLU_trend [-0.03:0.03 by=0.001]"))
...
mma <- ggpredict(m_m_bisqr, terms=c("AI_trend [-1:1 by=0.01]"))
...
mla <- ggpredict(m_l_bisqr, terms=c("AI_trend [-3:3 by=0.01]"))
...
#size
mst <- ggpredict(m_s_bisqr, terms=c("TPI_trend [-0.05:0.05 by=0.01]"))
...
msa <- ggpredict(m_s_bisqr, terms=c("AI_trend [-6:6 by=0.01]"))
